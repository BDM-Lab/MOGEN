...
public class GenomeGeneratorList implements OptimizedObject{
	...
	//private List<Contact> ctLst;
	private HashMap<Integer, Contact> ctMap;
	...
	public GenomeGeneratorList(String paraFile){
	...
	private void initialization() throws Exception{
		...
		//helper.divideDataSetWithTotal(ctLst.size(), numOfcores, lstSubDataSetId);
		...
		System.out.println("Number of poitns:" + n);
		System.out.println("Number of jobs:" + num_of_processors);
		
//		System.out.println("Size of each job!");		
//		for(int i = 0; i < lstSubDataSetId.size(); i++){
//			int to,be,en;
//			if (i == 0){
//				to = 0; 
//				be = 0; 
//				en = lstSubDataSetId.get(i);
//			}else{
//				to = 0; 
//				be = lstSubDataSetId.get(i - 1);
//				en = lstSubDataSetId.get(i);
//				
//			}
//			
...
//		System.out.println();
	...
	public void generateStructure() throws IOException,Exception{
		...
		while(i < NUM){
			...
			try{
				...
				//strVO = strScoring.calculateScore(str, ctLst,POS_MAX_DIST * zoomFactor,idToChr, NBR_OF_CHR);
				strVO = strScoring.calculateScore(str, ctMap,POS_MAX_DIST * zoomFactor,idToChr, NBR_OF_CHR);
	...
	private void readInputData() throws Exception{
		...
		try{
			...
			//ctLst = helper.readContactListAsList(INPUT_FILE,intra_thres,inter_thres,CHR_UPPER_BOUND_ID,lstPositions,idToChr);
			ctMap = helper.readContactListAsList(INPUT_FILE,intra_thres,inter_thres,CHR_UPPER_BOUND_ID,lstPositions,idToChr);
			...
			double avgIF = totalIF / count;
			for(Contact ct: ctMap.values()){
				ct.setIF(ct.getIF() / avgIF);
			}
			//
		...
		if (is_intra_thres_percent){
			...
			for(Contact ct : ctMap.values()){
				if (ct.getChr1() == ct.getChr2()){
					lst.add(ct.getIF());
				}
			}			
			Collections.sort(lst,Collections.reverseOrder());
			
			//compute the total intra contact
			int totalIntra = 0;
			for(int i = 0; i < n; i++){
				for(int j = i + 1; j < n; j++){
			...
			int index = (int)(totalIntra * INTRA_IF_THRESHOLD / 100);
			
			if (index >= lst.size()){
				intra_thres = 0.0; //don't remove anything because we need more than lst.size();
			}else{
				intra_thres = lst.get(index);
			}
			lst = null;
			...
			//for(Contact ct : ctLst){
			for(Contact ct : ctMap.values()){
				if (ct.getChr1() == ct.getChr2() && ct.getIF() < intra_thres){
					//ct.setIF(0.0);
					removeList.add(ct.getPos1() * n + ct.getPos2());
				}
			}
		...
		System.out.println("Computing intra_percent");
		//for(Contact ct : ctLst){
		for(Contact ct : ctMap.values()){
			if (ct.getChr1() == ct.getChr2()){
				totalPossible++;
				if (ct.getIF() < intra_thres){
					totalIntra++;
				}
		...
		System.out.println("Finsished computing intra_percent");
		...
		if (is_inter_thres_percent){
			...
			int index = (int)(totalInter * INTER_IF_THRESHOLD / 100);
			
			if (index >= lst.size()){
				inter_thres = 0.0;
			}else{
				inter_thres = lst.get(index);
			}
			...
			//for(Contact ct : ctLst){
			for(Contact ct : ctMap.values()){
				if (ct.getChr1() != ct.getChr2() && ct.getIF() < inter_thres){
					//ct.setIF(0.0);
					//ctMap.remove(ct.getPos1() * n + ct.getPos2());
					removeList.add(ct.getPos1() * n + ct.getPos2());
				}
			}
	...
	private void normalizeContactMT(){
		...
		ArrayList<Double> lstAdIF = new ArrayList<Double>();
		for(Contact ct : ctMap.values()){
				f = ct.getIF();
				if (Math.abs(ct.getPos1() - ct.getPos2()) == 1){
					lstAdIF.add(ct.getIF());
				}
		...
		//maxIF = (max1 + max2 + max3 + max4 + max5 + max6 + max7) / 7.0 ; //because all IFs are divided by avgIF
		maxIF = (max1 + max2) / 2.0 ; //because all IFs are divided by avgIF
	
		//using median to normalize
//		Collections.sort(lstAdIF);
//		maxIF = lstAdIF.get(lstAdIF.size() - 50);
//		try{
//			PrintWriter pw1 = new PrintWriter("test.txt");
//			for(Double d: lstAdIF){
//				pw1.println(d);
//			}
//			pw1.close();
//		}catch(Exception ex){
...
//		}
		//
		maxIF = 500;
		System.out.printf("Normalized adjacent contact, maxIF %.5f \n", maxIF);
		...
		for(int i = 0; i < n - 1; i++){
			...
			Contact ct = null;			
			if (ctMap.containsKey(val)){ // if contact exists				
				ct = ctMap.get(val);
				if (ct.getChr1() == ct.getChr2() && ct.getIF() < maxIF){
					ct.setIF(maxIF);
				}
				
			}else if (idToChr.get(i) == idToChr.get(i + 1)){//if not but i and i + 1 are in the same chromosome, make a new contact and add into the list
				ct = new Contact(i,idToChr.get(i),i + 1,idToChr.get(i), maxIF);
				ctMap.put(ct.getPos1() * n + ct.getPos2(), ct);
			}			
		}
		...
		System.out.println("Finished normalized adjacent contact");
		...
		normTotalIF = totalIF;
		System.out.printf("Total after norm:%.3f, average IF: %.3f \n", totalIF, avgIF);
	...
	private void logParameters(){
		...
		for(int i = 1; i < POS_MAX_DIST_WEIGHT.length; i++){
			for(int j = i; j < POS_MAX_DIST_WEIGHT.length; j++){
				logPW.println(i + " | " + j + ": " + POS_MAX_DIST_WEIGHT[i][j]);
			}
		}
		
		logPW.println("Weight to enfore minimum distance between fragments (POS_MIN_DIST_WEIGHT_FILE) : " + POS_MIN_DIST_WEIGHT_FILE);
		for(int i = 1; i < POS_MIN_DIST_WEIGHT.length; i++){
			for(int j = i; j < POS_MIN_DIST_WEIGHT.length; j++){
				logPW.println(i + " | " + j + ": " + POS_MIN_DIST_WEIGHT[i][j]);
			}
		}

		
		logPW.println("Weight to enfore non-contacts to be satisfied (NEG_MIN_DIST_WEIGHT_FILE) : " + NEG_MIN_DIST_WEIGHT_FILE);
		for(int i = 1; i < NEG_MIN_DIST_WEIGHT.length; i++){
			for(int j = i; j < NEG_MIN_DIST_WEIGHT.length; j++){
				logPW.println(i + " | " + j + ": " + NEG_MIN_DIST_WEIGHT[i][j]);
			}
		}

		
		logPW.println("Weight to enfore maximum distance between fragments (NEG_MAX_DIST_WEIGHT_FILE) : " + NEG_MAX_DIST_WEIGHT_FILE);
		for(int i = 1; i < NEG_MAX_DIST_WEIGHT.length; i++){
			for(int j = i; j < NEG_MAX_DIST_WEIGHT.length; j++){
				logPW.println(i + " | " + j + ": " + NEG_MAX_DIST_WEIGHT[i][j]);
			}
		}
...
//				count++;
	...
	class GradientCaculator extends Thread{
		...
		private Helper helperUtil = new Helper();
		...
		public void run(){
			...
			for(int i = beg; i <= end; i ++){
				for(int j = i + 1; j < n; j++){
					...
					dist = helperUtil.calEuclidianDist(structure[i * 3],structure[i * 3 + 1], structure[i * 3 + 2],
							 structure[j * 3],structure[j * 3 + 1], structure[j * 3 + 2]);
					
					if (ctMap.containsKey(val) && ctMap.get(val).getIF() > 0) {
				
						ct = ctMap.get(val);
						 
//						if (i != ct.getPos1() ||  j != ct.getPos2()){
//							System.err.printf("Indices are wrong: %d %d vs. %d %d \n" ,i,j,ct.getPos1(),ct.getPos2());
//							System.exit(1);
//						}
//						
//						if (chr1 != ct.getChr1() ||  chr2 != ct.getChr2()){
//							System.err.printf("Indices are wrong: %d %d vs. %d %d \n" ,chr1,chr2,ct.getChr1(),ct.getChr2());
//							System.exit(1);
//						}
						...
						tanh2 = helperUtil.tanh(dist - POS_MIN_DIST);
						...
						if (chr1 == chr2 && j - i == 1){
							...
							tanh1 = helperUtil.tanh(ADJACENT_DIST - dist);				
							 
						}else { 
							//positive contact, the distance should be less than POS_MAX_DIST
							tanh1 = helperUtil.tanh(POS_MAX_DIST - dist);
						}
	...
	public static void main(String[] args) throws Exception{
		String paraFile = "parameters_hic2014_gm12878_chr2.txt";
		...
		GenomeGeneratorList generator = new GenomeGeneratorList(paraFile);
