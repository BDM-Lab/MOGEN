...
import static valueObject.Constants.*;
...
public class GradientAscent {
	...
	public void performGradientAscent(int max_iteration) throws Exception{
		...
		while(! isConvergence() && count < max_iteration){
			...
			if (verbose){
				System.out.printf("Iteration %1$d, current objective function:%2$.7f, length of gradient %3$.7f\n",count,objectiveFn,gradientNorm);
			}
	...
	private double lineSearch(double[] variables, double[] direction, double currentFn) throws Exception{
		...
		if (alpha <= NEAR_ZERO){
			System.err.println("Error in the objective function, try to reduce the initial learnign rate !!! \n "
					+ "If it is already small, stop the program and call up Tuan! :( ");
			throw new Exception("Error in the objective function, try to reduce the initial learnign rate !!! \n "
					+ "If it is already small, stop the program and call up Tuan! :( ");
		}
